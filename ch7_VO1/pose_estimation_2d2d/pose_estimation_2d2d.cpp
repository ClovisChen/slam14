...
// pixel coordinate -> camera normalized coordinate
...
int main( int argc, char** argv )
{
    ...
    //-- verify the pole condinement
    Mat K = Mat(3,3,CV_32F);
    K = ( Mat_<double> ( 3,3 ) << 520.9, 0, 325.1, 0, 521.0, 249.7, 0, 0, 1 );
    for ( DMatch m: matches )
    {
        Point2d pt1 = pixel2cam( keypoints_1[ m.queryIdx ].pt, K );
        Mat y1 = Mat(3,3,CV_32F);
        y1 = (Mat_<double>(3,1) << pt1.x, pt1.y, 1);
        Point2d pt2 = pixel2cam( keypoints_2[ m.trainIdx ].pt, K );
        Mat y2 = Mat(3,3,CV_32F);
        y2 = (Mat_<double>(3,1) << pt2.x, pt2.y, 1);
        Mat d = y2.t() * t_x * R * y1;
                            ...
                            Mat& R, Mat& t )
{
    ...
    //-- Essential
    Point2d principal_point ( 325.1, 249.7 );				//principla_point, TUM dataset calibration value
    int focal_length = 521;						//focal_length, TUM dataset calibration value
