...
public:
    ...
    bool read( istream& is ){
        ...
        for( int i=0; i<7; i++ )
            is>>data[i];
        setEstimate( SE3 (Eigen::Quaterniond ( data[6], data[3], data[4], data[5] ), Eigen::Vector3d ( data[0], data[1], data[2] ) ));}
...
int main ( int argc, char** argv ){
    if ( argc != 2 ){
        cout<<"Usage: pose_gragh_g2o_SE3_lie sphere.g2o"<<endl;
        return 1;
    }
    ifstream fin( argv[1] );
    if ( !fin ){
    ...
    /// cd cmake-build-debug g2o_viewer result_lie.g2o
